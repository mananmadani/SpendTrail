// Data Management - Now using IndexedDB
async function getData() {
  try {
    return await dbInstance.getData();
  } catch (error) {
...
async function migrateFromLocalStorage() {
  const migrationKey = 'SpendTrail-migrated-to-indexeddb';
  ...
  try {
    ...
    if (oldData) {
      ...
      console.log(`📦 Found ${incomeCount} income and ${expenseCount} expense entries`);
      
      if (incomeCount > 0 || expenseCount > 0) {
        await setData(data);
        console.log('✅ Migration completed successfully!');
        
        localStorage.setItem('SpendTrail-data-backup-pre-v37', oldData);
        
        showToast(`Upgraded! Now supporting unlimited storage`, 'success');
      }
    }
    ...
    showToast('Migration error. Using IndexedDB anyway.', 'error');
    localStorage.setItem(migrationKey, 'true');
  }
}
...
document.getElementById('add-form').addEventListener('submit', async function(e) {
  ...
  if (currentTab === 'home') await loadHome();
...
// Edit & Delete
async function editEntry(dataType, index) {
  const data = await getData();
...
async function saveEdit(dataType, index) {
  const data = await getData();
  ...
  if (currentTab === 'home') {
    ...
    if (currentOverlayContext.type === 'allEntries') {
      await showAllEntries(currentOverlayContext.filter);
    } else if (currentOverlayContext.type === 'category') {
      await showCategoryDetails(currentOverlayContext.category, currentOverlayContext.filter);
    } else {
      closeOverlay();
    }
  } else {
...
async function deleteEntry(dataType, index) {
  ...
  if (currentTab === 'home') {
    ...
    if (currentOverlayContext.type === 'allEntries') {
      ...
      const remainingInCategory = [
        ...updatedData.income.filter(e => e.category === deletedCategory),
        ...updatedData.expenses.filter(e => e.category === deletedCategory)
      ];
...
// All Entries
async function showAllEntries(filter = 'all') {
...
// Ledger
async function showLedger(filter = 'all') {
...
async function showCategoryDetails(category, filter = 'all') {
...
async function exportStatementPDF() {
  ...
  doc.setFontSize(12);
  doc.text(`Rs. ${periodIncome.toFixed(2)}`, 47.5, y + 22, { align: 'center' });
  ...
  doc.setFontSize(12);
  doc.text(`Rs. ${periodExpense.toFixed(2)}`, 107.5, y + 22, { align: 'center' });
  ...
  doc.setFontSize(12);
  const balanceText = periodBalance >= 0 ? `Rs. ${periodBalance.toFixed(2)}` : `-Rs. ${Math.abs(periodBalance).toFixed(2)}`;
  ...
  all.forEach((item, index) => {
    ...
    const amount = `${item.type === 'income' ? '+' : '-'}Rs. ${Number(item.amount).toFixed(2)}`;
...
async function showAnalytics(days = 30, type = 'expense') {
  ...
  container.innerHTML = categories.map(([cat, amt]) => {
    ...
    return `
      <div class="category-bar">
        ...
        <div class="category-bar-amount" style="color:${amountColor}">₹${amt.toFixed(0)}</div>
      </div>
    `;
  }).join('');
}
  ...
  const content = `
    <div class="chart-container">
      ...
      ${window.othersData.map(([cat, amt]) => `
        <div class="category-bar">
          <div class="category-bar-label">${cat}</div>
          <div class="category-bar-amount" style="color:${amountColor}">₹${amt.toFixed(2)}</div>
        </div>
      `).join('')}
    </div>
...
async function backupData() {
  const data = await getData();
  const backup = { ...data, backupDate: new Date().toISOString(), version: "3.7" };
...
async function encryptedBackup() {
  ...
  const data = await getData();
  const backup = { ...data, backupDate: new Date().toISOString(), version: "3.7", encrypted: true };
    ...
    reader.onload = async function(event) {
      try {
        ...
        if (confirm(`Restore ${backup.income.length} income and ${backup.expenses.length} expense entries?`)) {
          await setData({ income: backup.income, expenses: backup.expenses });
          await loadHome();
...
async function exportPDF() {
  ...
  doc.text('Income', 47.5, y + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.text(`Rs. ${totalIncome.toFixed(2)}`, 47.5, y + 22, { align: 'center' });
  ...
  doc.text('Expense', 107.5, y + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.text(`Rs. ${totalExpense.toFixed(2)}`, 107.5, y + 22, { align: 'center' });
  ...
  doc.text('Balance', 165, y + 12, { align: 'center' });
  doc.setFontSize(12);
  const balanceText = balance >= 0 ? `Rs. ${balance.toFixed(2)}` : `-Rs. ${Math.abs(balance).toFixed(2)}`;
  doc.text(balanceText, 165, y + 22, { align: 'center' });
  ...
  all.forEach((item, index) => {
    if (y > 270) {doc.setTextColor(150, 150, 150);
      doc.setFontSize(8);
      doc.text(`Page ${pageNumber}`, 105, 290, { align: 'center' });
...
async function deleteAllData() {
  if (confirm('Delete ALL data? Cannot be undone!')) {
    if (confirm('Absolutely sure?')) {
      await dbInstance.clear('income');
      await dbInstance.clear('expenses');
      localStorage.removeItem('SpendTrail-data');
      localStorage.removeItem('SpendTrail-data-backup-pre-v37');
      await loadHome();
  ...
  const content = `
    <div style="line-height:1.7;color:#424242;">
      ...
      <div style="margin-bottom:28px;">
        ...
        <p style="margin-bottom:12px;">SpendTrail is designed with your privacy as the top priority. All your financial data, including income entries, expense records, categories, notes, and transaction history, is stored exclusively on your device using IndexedDB.</p>
      ...
      <div style="margin-bottom:28px;">
        ...
        <ul style="margin-left:20px;margin-bottom:12px;">
          <li style="margin-bottom:8px;"><strong>Local Storage:</strong> Data is stored in your browser's secure IndexedDB, isolated from other websites and applications.</li>
      ...
      <p style="margin-top:24px;font-size:13px;color:#757575;text-align:center;">Last Updated: December 2025 | Version 3.7</p>
    </div>
  `;
...
async function initApp() {
  try {
    ...
    console.log('🚀 Initializing SpendTrail v3.7...');
