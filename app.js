...
function openOverlay(title, content) {
  ...
  if (!isAlreadyOpen) {
    // Only animate on first open — not on filter switches within same overlay
...
function backupData() {
  ...
  const backup = { ...data, backupDate: new Date().toISOString(), version:'4.0.8', profileName: profile?.name||'Personal' };
...
function encryptedBackup() {
  ...
  const data = getData(), profile = getActiveProfile();
  const backup = { ...data, backupDate:new Date().toISOString(), version:'4.0.8', encrypted:true, profileName:profile?.name||'Personal' };
  ...
  const content = `
    <div style="line-height:1.7;color:#424242;">
      ...
      <div style="text-align:center;padding-top:8px;padding-bottom:4px;">
        <p style="font-size:12px;color:#BDBDBD;margin:0;">Last Updated: March 2026 &nbsp;|&nbsp; Version 4.0.8</p>
      </div>
...
window.addEventListener('popstate', function(event) {
  ...
  if (overlay.classList.contains('active')) {
    ...
    if (state && state.view === 'overlay') {
      ...
      // else: already on correct tab — browser handles exit naturally at base state
    }
  }
});
