...
function getCurrencySymbol() {
  return localStorage.getItem('SpendTrail-currency') || '₹';
}

function setCurrencySymbol(symbol) {
  localStorage.setItem('SpendTrail-currency', symbol);
}
...
// Helper function to sort transactions by date then timestamp
...
function filterAllEntries(query) {
  ...
  const entries = document.querySelectorAll('#all-entries-list .entry-item');
  entries.forEach(entry => {
    const category = entry.getAttribute('data-category');
    const note = entry.getAttribute('data-note');
    const matches = category.includes(q) || note.includes(q);
...
function showLedger(filter = 'all') {
  ...
  openOverlay('Ledger', `
    ...
    return `<button class="insight-option" data-category="${cat.toLowerCase()}" onclick="showCategoryDetails('${cat}', 'all')"><span class="option-icon">📁</span><div style="flex:1;"><div class="option-text">${cat}</div><div style="font-size:13px;color:#757575;margin-top:2px;">Income: ${currency}${catIncome.toFixed(2)} | Expense: ${currency}${catExpense.toFixed(2)} | ${catCount} entries</div></div><span class="option-arrow">→</span></button>`;
  }).join('')}</div>`);
}
...
function generateStatement() {
  ...
  const data = getData();
  const currency = getCurrencySymbol();
  let all = [...data.income.map((e, i) => ({...e, type: 'income', dataType: 'income', index: i})), ...data.expenses.map((e, i) => ({...e, type: 'expense', dataType: 'expenses', index: i}))].filter(i => i.date >= start && i.date <= end);
  all = sortTransactions(all);
  document.getElementById('statement-result').innerHTML = !all.length ? '<div class="empty-state"><div class="empty-icon">📋</div><div class="empty-text">No entries for this period</div></div>' : `<div class="transaction-list">${all.map(item => `<div style="background:#FFF;border-radius:14px;padding:16px;box-shadow:0 2px 6px rgba(0,0,0,0.05);margin-bottom:10px;"><div style="display:flex;align-items:center;gap:14px;"><div style="width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;background:${item.type === 'income' ? '#E8F5E9' : '#FFEBEE'};">${item.type === 'income' ? '↗' : '↘'}</div><div style="flex:1;min-width:0;"><div style="font-size:15px;font-weight:600;color:#1A1A1A;margin-bottom:3px;">${item.category}</div><div style="font-size:13px;color:#757575;">${formatDate(item.date)}${item.note ? ' • ' + item.note : ''}</div></div><div style="font-size:18px;font-weight:700;color:${item.type === 'income' ? '#4CAF50' : '#F44336'};white-space:nowrap;">${item.type === 'income' ? '+' : '-'}${currency}${item.amount}</div></div></div>`).join('')}</div>`;
}
function exportStatementPDF() {
  ...
  if (all.length === 0) {
    showToast('No entries in this period', 'error');
    return;
  }
  ...
  // Convert currency symbol to PDF-safe text
  let pdfCurrency = currency;
  if (currency === '₹') pdfCurrency = 'Rs.';
  else if (currency === '¥' || currency === '元') pdfCurrency = 'Y';
  else if (currency === '£') pdfCurrency = 'GBP';
  else if (currency === '€') pdfCurrency = 'EUR';
  else if (currency === '฿') pdfCurrency = 'THB';
  else if (currency === '₨') pdfCurrency = 'PKR';
  else if (currency === '৳') pdfCurrency = 'BDT';
  else if (currency === 'د.إ') pdfCurrency = 'AED';
  else if (currency === '﷼') pdfCurrency = 'SAR';
  else if (currency === 'ر.ق') pdfCurrency = 'QAR';
  else if (currency === 'zł') pdfCurrency = 'PLN';
  else if (currency === '₺') pdfCurrency = 'TRY';
  else if (currency === '₽') pdfCurrency = 'RUB';
  else if (currency === '₩') pdfCurrency = 'KRW';
  ...
  doc.text('SpendTrail', 105, 20, { align: 'center' });
  
  doc.setFontSize(14);
  doc.setFont(undefined, 'normal');
  doc.text('Custom Statement', 105, 30, { align: 'center' });
  
  doc.setFontSize(10);
  const startDate = new Date(start).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
  const endDate = new Date(end).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
  ...
  doc.roundedRect(20, y + 5, 55, 25, 2, 2, 'F');
  doc.setTextColor(76, 175, 80);
  doc.setFontSize(10);
  doc.setFont(undefined, 'bold');
  doc.text('Income', 47.5, y + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.text(`${pdfCurrency} ${periodIncome.toFixed(2)}`, 47.5, y + 22, { align: 'center' });
  
  doc.setFillColor(255, 235, 238);
  doc.roundedRect(80, y + 5, 55, 25, 2, 2, 'F');
  doc.setTextColor(244, 67, 54);
  doc.setFontSize(10);
  doc.setFont(undefined, 'bold');
  doc.text('Expense', 107.5, y + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.text(`${pdfCurrency} ${periodExpense.toFixed(2)}`, 107.5, y + 22, { align: 'center' });
  
  doc.setFillColor(227, 242, 253);
  doc.roundedRect(140, y + 5, 50, 25, 2, 2, 'F');
  doc.setTextColor(102, 126, 234);
  doc.setFontSize(10);
  doc.setFont(undefined, 'bold');
  doc.text('Balance', 165, y + 12, { align: 'center' });
  doc.setFontSize(12);
  const balanceText = periodBalance >= 0 ? `${pdfCurrency} ${periodBalance.toFixed(2)}` : `-${pdfCurrency} ${Math.abs(periodBalance).toFixed(2)}`;
  doc.text(balanceText, 165, y + 22, { align: 'center' });
  ...
  doc.setFontSize(9);
  doc.text(`Total: ${all.length} entries`, 15, y + 6);
  ...
  doc.text('Category', 45, y + 5);
  doc.text('Note', 95, y + 5);
  ...
  all.forEach((item, index) => {
    ...
    const amount = `${item.type === 'income' ? '+' : '-'}${pdfCurrency} ${Number(item.amount).toFixed(2)}`;
...
let currentAnalyticsType = 'expense';
let pieChart = null;
let trendChart = null;
...
function showAnalytics(days = 30, type = 'expense') {
  ...
  const dataToAnalyze = type === 'income' ? filteredIncome : filteredExpenses;
  ...
  const content = `
    ...
    ${pieData.length === 0 ? '<div class="empty-state"><div class="empty-icon">📊</div><div class="empty-text">No ' + type + ' data</div><div class="empty-subtext">Add some ' + type + 's to see analytics</div></div>' : `
      <div class="chart-container">
        <div class="chart-title">${type === 'income' ? 'Income' : 'Expense'} Breakdown</div>
  ...
  if (pieData.length > 0) {
    setTimeout(() => {
      renderPieChart(pieData, totalAmount, type);
      renderCategoryBars(sortedCategories, totalAmount, type);
      renderTrendChart(data, days);
    }, 100);
  }
  
  window.othersData = others;
  window.currentAnalyticsType = type;
}
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              ...
              const percent = ((value / total) * 100).toFixed(1);
              return `${currency}${value.toFixed(2)} (${percent}%)`;
            }
          }
        }
      }
    ...
    return `
      <div class="category-bar">
        ...
        <div class="category-bar-fill">
          <div class="category-bar-progress" style="width:${percent}%;background:${gradientColor}"></div>
        </div>
        <div class="category-bar-amount" style="color:${amountColor}">${currency}${amt.toFixed(0)}</div>
      </div>
    `;
  }).join('');
}
...
function showOthersBreakdown() {
  ...
  const type = window.currentAnalyticsType || 'expense';
  const currency = getCurrencySymbol();
  const amountColor = '#667EEA';
  ...
  const content = `
    <div class="chart-container">
      ...
      ${window.othersData.map(([cat, amt]) => `
        <div class="category-bar">
          <div class="category-bar-label">${cat}</div>
          <div class="category-bar-amount" style="color:${amountColor}">${currency}${amt.toFixed(2)}</div>
        </div>
      `).join('')}
    </div>
...
function backupData() {
  const data = getData();
  const backup = { ...data, backupDate: new Date().toISOString(), version: "3.8" };
...
function encryptedBackup() {
  ...
  const data = getData();
  const backup = { ...data, backupDate: new Date().toISOString(), version: "3.8", encrypted: true };
...
function exportPDF() {
  ...
  doc.setFontSize(10);
  doc.text(`Generated on ${new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}`, 105, 35, { align: 'center' });
  ...
  doc.text('Income', 47.5, y + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.text(`${pdfCurrency} ${totalIncome.toFixed(2)}`, 47.5, y + 22, { align: 'center' });
  ...
  doc.text('Expense', 107.5, y + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.text(`${pdfCurrency} ${totalExpense.toFixed(2)}`, 107.5, y + 22, { align: 'center' });
  ...
  doc.text('Balance', 165, y + 12, { align: 'center' });
  doc.setFontSize(12);
  const balanceText = balance >= 0 ? `${pdfCurrency} ${balance.toFixed(2)}` : `-${pdfCurrency} ${Math.abs(balance).toFixed(2)}`;
  doc.text(balanceText, 165, y + 22, { align: 'center' });
  ...
  doc.setFontSize(16);
  doc.setFont(undefined, 'bold');
  doc.text('All Transactions', 15, y);
...
function showCurrencySettings() {
  ...
  const currentCode = Object.keys(CURRENCY_SYMBOLS).find(key => CURRENCY_SYMBOLS[key] === current) || 'INR';
      ...
      <div class="form-group">
        ...
        <select id="currency-select" style="background:#FFF;border:2px solid #E0E0E0;border-radius:12px;padding:14px 16px;font-size:16px;width:100%;">
          ${Object.entries(CURRENCY_SYMBOLS).map(([code, symbol]) => 
            `<option value="${symbol}" ${symbol === current ? 'selected' : ''}>${code} - ${symbol}</option>`
          ).join('')}
        </select>
      </div>
  ...
  openOverlay('Currency Symbol', content);
}
  ...
  const content = `
    <div style="line-height:1.7;color:#424242;">
      ...
      <div style="margin-bottom:28px;">
        ...
        <p style="margin-bottom:12px;">SpendTrail v3.8 introduces multi-currency symbol support:</p>
        <ul style="margin-left:20px;">
          <li style="margin-bottom:8px;">Your selected currency symbol is stored locally on your device</li>
      ...
      <div style="margin-bottom:28px;">
        ...
        <p style="margin-top:12px;">Please note that since SpendTrail operates entirely locally without any backend infrastructure, we cannot access, view, or recover your data remotely. For data recovery, you must have a backup file.</p>
      </div>
      ...
      <p style="margin-top:24px;font-size:13px;color:#757575;text-align:center;">Last Updated: February 2026 | Version 3.8</p>
    </div>
  `;
