...
function renderRecentList() {
    const data = getData();
    // Combine with proper order tracking
    const incomeWithOrder = data.income.map((e, idx) => ({...e, type: 'Income', insertOrder: idx}));
    const expenseWithOrder = data.expenses.map((e, idx) => ({...e, type: 'Expense', insertOrder: data.income.length + idx}));
    let all = [...incomeWithOrder, ...expenseWithOrder];
    
    // Sort by date (newest first), then by insert order (newest first)
    all.sort((a, b) => {
...
document.getElementById('export-pdf-btn').addEventListener('click', function() {
    ...
    const incomeWithOrder = data.income.map((e, idx) => ({...e, type: 'Income', insertOrder: idx}));
    const expenseWithOrder = data.expenses.map((e, idx) => ({...e, type: 'Expense', insertOrder: data.income.length + idx}));
    let allEntries = [...incomeWithOrder, ...expenseWithOrder].sort((a, b) => {
