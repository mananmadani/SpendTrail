    ...
    <div id="ledger-categories" class="insights-menu">
      ${cats.map(cat => {
        ...
        return `<button class="insight-option" data-category="${cat.toLowerCase()}" onclick="showCategoryDetails('${cat}','all')">
          ...
          <div style="flex:1;"><div class="option-text">${cat}</div><div style="font-size:13px;color:#757575;margin-top:2px;">Income: ${currency}${formatAmount(catIncome)} | Expense: ${currency}${formatAmount(catExpense)} | ${catCount} entries</div></div>
...
function renderTrendChart(data, days) {
  ...
  trendChart = new Chart(canvas, {
    ...
    options:{ responsive:true, maintainAspectRatio:true, interaction:{intersect:false,mode:'index'},
      ...
      scales:{ y:{beginAtZero:true,ticks:{callback:v=>currency+formatAmount(v)}}, x:{ticks:{maxRotation:45,minRotation:45}} }
    }
  });
}
...
function showPrivacy() {
  const content = `
    <div style="line-height:1.7;color:#424242;">
      ...
      <p style="margin-top:24px;font-size:13px;color:#757575;text-align:center;">Last Updated: February 2026 | Version 3.9</p>
    </div>`;
