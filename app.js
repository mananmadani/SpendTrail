...
function getSettings() { 
  return JSON.parse(localStorage.getItem('SpendTrail-settings') || '{"currency":"INR"}'); 
}
function setSettings(settings) { 
  localStorage.setItem('SpendTrail-settings', JSON.stringify(settings)); 
}
  ...
  const content = `
    <div class="currency-selector">
      ${Object.entries(CURRENCIES).map(([code, symbol]) => `
        <button class="currency-option ${code === currentCurrency ? 'selected' : ''}" onclick="changeCurrency('${code}')">
          <span class="currency-symbol">${symbol}</span>
          <span class="currency-code">${code}</span>
          ${code === currentCurrency ? '<span class="currency-check">✓</span>' : ''}
        </button>
      `).join('')}
    </div>
  `;
  
  openOverlay('Select Currency', content);
}
...
function exportStatementPDF() {
  ...
  all.forEach(item => { 
    ...
    doc.text(`${item.type === 'income' ? '+' : '-'}${currency}${item.amount} | ${item.category} | ${item.date}${noteText}`, 20, y);
    y += 7; 
  });
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              ...
              const percent = ((value / total) * 100).toFixed(1);
              return `${symbol}${value.toFixed(2)} (${percent}%)`;
            }
          }
        }
      }
  ...
  container.innerHTML = categories.map(([cat, amt]) => {
    ...
    return `
      <div class="category-bar">
        ...
        <div class="category-bar-amount">${symbol}${amt.toFixed(0)}</div>
      </div>
    `;
  }).join('');
}
  ...
  const content = `
    <div class="chart-container">
      ...
      ${window.othersData.map(([cat, amt]) => `
        <div class="category-bar">
          <div class="category-bar-label">${cat}</div>
          <div class="category-bar-amount">${symbol}${amt.toFixed(2)}</div>
        </div>
      `).join('')}
    </div>
...
function exportPDF() {
  ...
  doc.setFontSize(10);
  doc.text(`Income: ${currency}${totalIncome.toFixed(2)} | Expense: ${currency}${totalExpense.toFixed(2)} | Balance: ${currency}${(totalIncome - totalExpense).toFixed(2)}`, 20, y); y += 15;
  let all = [...data.income.map(e => ({...e, type: 'Income'})), ...data.expenses.map(e => ({...e, type: 'Expense'}))].sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
  all.forEach(item => { 
    if (y > 270) { doc.addPage(); y = 20; } 
    const noteText = item.note ? ` | ${item.note}` : '';
    doc.text(`${item.type} | ${currency}${item.amount} | ${item.category} | ${item.date}${noteText}`, 20, y);
    y += 7; 
  });
...
function showPrivacy() {
  const content = `
    <div style="line-height:1.7;color:#424242;">
      ...
      <p style="margin-top:24px;font-size:13px;color:#757575;text-align:center;">Last Updated: November 2025 | Version 3.5</p>
    </div>
  `;
