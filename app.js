...
function backupData() {
  ...
  const backup = { ...data, backupDate: new Date().toISOString(), version:'4.0.5', profileName: profile?.name||'Personal' };
...
function encryptedBackup() {
  ...
  const data = getData(), profile = getActiveProfile();
  const backup = { ...data, backupDate:new Date().toISOString(), version:'4.0.5', encrypted:true, profileName:profile?.name||'Personal' };
...
window.addEventListener('popstate', function() {
  ...
  if (overlay.classList.contains('active')) {
    ...
    // On a non-home tab — go back to Home
    currentTab = 'home';
    document.querySelectorAll('.tab-content').forEach(t => t.classList.remove('active'));
  ...
  // else: already on Home with no overlay — browser handles exit naturally
});
