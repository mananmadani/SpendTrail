...
function exportStatementPDF() {
  ...
  doc.setFontSize(10);
  all.forEach(item => { if (y > 270) { doc.addPage(); y = 20; } doc.text(`${item.type === 'income' ? '+' : '-'}₹${item.amount} | ${item.category} | ${item.date}`, 20, y); y += 7; });
...
function showAnalytics(days = 30) {
  ...
  const content = `
    <div class="time-filter">
      <button class="${days === 7 ? 'active' : ''}" onclick="showAnalytics(7)">Week</button>
      <button class="${days === 30 ? 'active' : ''}" onclick="showAnalytics(30)">Month</button>
      <button class="${days === 90 ? 'active' : ''}" onclick="showAnalytics(90)">3 Months</button>
    </div>
...
function exportPDF() {
  ...
  all.forEach(item => { if (y > 270) { doc.addPage(); y = 20; } doc.text(`${item.type} | ₹${item.amount} | ${item.category} | ${item.date}`, 20, y); y += 7; });
...
function showPrivacy() {
  const content = `
    <div style="line-height:1.7;color:#424242;">
      ...
      <p style="margin-top:24px;font-size:13px;color:#757575;text-align:center;">Last Updated: November 2025 | Version 3</p>
    </div>
  `;
